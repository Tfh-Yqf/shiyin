<template>
	<view style="height: 100%;">
		...
		<view  style="background-color: #ffffff;flex-shrink: 0;display: flex;flex-direction: column;" :style="{height:height.shang+'px'}">
			...
			<view style="display: flex;flex-direction: row;width: 100%;height: 30px;align-items: center;justify-content: space-between;">
				...
				<view style="display: flex;flex-direction: row;height: 30px;align-items: center;flex-shrink: 0;" >
					<view v-if="!start&&!end&&!show_ans" @click="start_exam()" style="margin-right: 40rpx;background-color: #ff7599;height: 30px;width: 80px;border-radius: 10upx;text-align: center;">
						<text>开始考试</text>
					</view>
					<view v-if="start&&!end&&!show_ans" @click="start_exam()" style="margin-right: 40rpx;background-color:#62b8af;height: 30px;width: 160px;border-radius: 10upx;text-align: center;">
						<text>考试进行中{{((300-countdown)*10/9).toFixed(2)}}%</text>
						<text></text>
					</view>
					<view v-if="end&&!start&&!show_ans" @click="tijiao()" style="margin-right: 40rpx;background-color: #ff7599;height: 30px;width: 80px;border-radius: 10upx;text-align: center;">
						<text>结束考试</text>
					</view>
					<view v-if="show_ans" @click="close_pop()" style="margin-right: 40rpx;background-color:#62b8af;height: 30px;width: 80px;border-radius: 10upx;text-align: center;">
						<text>离开</text>
					</view>
				</view>
		...
		<uni-popup ref="result" :mask-click="false">
			<view style="background-color: #ffffff;border-radius: 20upx;display: flex;flex-direction: column;z-index: 99;" :style="{width:width.pop+'px',height:height.pop+'px'}">
				<view style="display: flex;flex-direction: column;background-color: #62b8af;color: #ffffff;" :style="{height:height.pop*0.8+'px'}">
					<view style="width: 100%;display: flex;flex-direction: row-reverse;height: 15px;">
						<image @click="close_pop()" src="../../../static/tingyin/chahao.png" style="width: 20px;margin-right: 10px;" mode="widthFix"></image>
					</view>
					<view style="width: 100%;text-align: center;">
						<text style="">答题结果</text>
					</view>
					<view style="display: flex;flex-direction: row;width: 100%;">
						<view style="display: flex;flex-direction: column;width: 60%;flex-shrink: 0;">
							<view style="display: flex;flex-direction: row;">
								<text style="width: 50%;margin-left: 10%;">正确率:</text>
								<text>{{finall_res.zhengque_per.toFixed(2)}}%</text>
							</view>
							<view style="display: flex;flex-direction: row;">
								<text style="width: 50%;margin-left: 10%;">正确个数:</text>
								<text>{{finall_res.right}}</text>
							</view>
							<view style="display: flex;flex-direction: row;">
								<text style="width: 50%;margin-left: 10%;">错误个数:</text>
								<text>{{finall_res.wrong}}</text>
							</view>
							<view style="display: flex;flex-direction: row;">
								<text style="width: 50%;margin-left: 10%;">未答个数:</text>
								<text>{{finall_res.empty}}</text>
							</view>
							<view style="display: flex;flex-direction: row;">
								<text style="width: 50%;margin-left: 10%;">用时:</text>
								<text>{{cost_time}}秒</text>
							</view>
						</view>
						<view style="text-align: center;display: flex;flex-direction: column;justify-content: center;width: 40%;flex-shrink: 0;">
							<view style="display: flex;flex-direction: row;justify-content: center;width: 100%;text-align: center;">
								<text style="font-size: 40px;color: crimson;">{{finall_res.grade}}分</text>
							</view>
				...
				<view style="display: flex;flex-direction: row;padding: 10px;">
					<button size="mini" type="warn" @click="close_pop()" >关闭</button>
					<button size="mini" type="primary" @click="cuowu_xiangqing()">查看错误详情</button>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			this.data = uni.getStorageSync('exam_chuancan');
			this.diff=this.data.diff;
			if(this.diff==0)	this.diff_name='简单';
			else if(this.diff==1)	this.diff_name='中等';
			else 	this.diff_name='困难';
		...
		methods: {
			...
			queding_tijiao(){
				...
				const sql_data = {
					...
					all_dian:JSON.stringify(this.storage_dian),
					diff:this.diff_name,
					music_link:this.data.data,
					cost_time:this.cost_time,
					empty:data.empty
				}
				if(uni.getStorageSync('tingyin_exam_foot')==false)
					var all_data = [];
				else
					var all_data = uni.getStorageSync('tingyin_exam_foot');
				all_data.push(sql_data);
				uni.setStorageSync('tingyin_exam_foot',all_data);
				console.log(sql_data);
				uni.request({
					url:this.server_url+'/exam_upload',
					method:'POST',
					data:sql_data,
					success: (res) => {
						console.log(res);
			...
			tijiao(){
				...
				// 计算是否全部作答
				var cnt = 0;
				for(var i=1;i<this.storage_dian.length;i++)
					for(var j=0;j<this.storage_dian[i].length;j++)
						cnt += this.storage_dian[i][j].length;
				console.log(cnt);
